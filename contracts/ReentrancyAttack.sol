...
contract ReentrancyAttack {
    ReentrancyBank public bank; // Bank合约地址

    // 初始化Bank合约地址
    constructor(ReentrancyBank _bank) {
        bank = _bank;
    }

    // 回调函数, 或者 fallback 用于重入攻击Bank合约, 反复的调用目标的withdraw函数
...
// 利用 重入锁 防止重入攻击
contract ReentrancyProtectedBank {
    mapping(address => uint256) public balanceOf;
